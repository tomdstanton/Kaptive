...
from .seq import Record, Feature
from .io import SeqFile, ReadSet
...
class ExternalProgramWarning(KaptiveWarning):
    pass
...
class ExternalProgram:
    ...
    def version(self) -> str:
        if not self._version:
            self._version = self._run('--version')[0].strip().split(maxsplit=1)[-1]
...
class Minimap2(ExternalProgram):
    ...
    Examples:
        >>> from kaptive.external import Minimap2
    ...
    def build_index(
        ...
        """
        arg, pipe = _validate_seqs(*targets)
        if not config:
    ...
    def align(
        ...
        """
        query_arg, query_pipe = _validate_seqs(*queries)
        if not config:
            config = self._align_config
        if targets:
            target_arg, target_pipe = _validate_seqs(*targets)
...
def _validate_seqs(*seqs: Union[str, Path, SeqFile, Record, Feature], pipe_arg: str = '-') -> tuple[
    ...
    for i in seqs:
        if isinstance(i, (Path, SeqFile)):
            ...
            if pipe:
                raise ExternalProgramError('Cannot mix pipe-able inputs with file-like inputs')
            ...
            if isinstance(i, str):
                if not i.startswith('>') and not i.startswith('@'):
                    print(i)
                    raise ExternalProgramError('If input is a string, it must be in sequence format')
                pipe += i
            else:
                pipe += format(i, 'fasta')
