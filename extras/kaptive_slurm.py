...
def main():
    ...
    using_start_end_margin = args.start_end_margin != main_parser.get_default(
        "start_end_margin"
    )
    ...
    using_min_assembly_piece = args.min_assembly_piece != main_parser.get_default(
        "min_assembly_piece"
    )
    ...
    for assembly_filename in args.assembly:
        ...
        if args.allelic_typing:
            kaptive_command += " --allelic_typing " + os.path.abspath(
                args.allelic_typing
            )
...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Kaptive - SLURM job generator",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--walltime", type=str, required=False, help="wall time", default="0-0:10"
    )
    parser.add_argument(
        "--memory", type=str, required=False, help="memory (Megabytes)", default="2048"
    )
